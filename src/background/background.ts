...
browser.runtime.onConnect.addListener(function (port) {
  assert(
    [process.env.MESSAGING_PORT, process.env.PORT_EXTENSION].includes(
      port.name
    ),
  ...
  chrome.runtime.onMessage.addListener(async (msg) => {
    if (msg.type === Messages.RevokeDapp) {
      authorizedDapps = authorizedDapps.filter(
        (item) => item !== msg.payload.dappName
      );
    }
    ...
    if (msg.type === Messages.DappAuthRequest) {
      try {
        ...
        if (
          authorizedDapps.includes(dappName) ||
          declinedDapps.includes(dappName)
        )
          return;
        // POPUP_CONTENT popup content will assure that request is coming from popup
        if (
          msg.payload.approved &&
          msg?.payload?.POPUP_CONTENT === process.env.POPUP_CONTENT
        ) {
          pendingRequests = [];
      ...
      try {
        ...
        if (
          ...
          const pair = keyPairs.find((pair) => {
            return (
              recodeToPolkadotAddress(pair.address) ===
              recodeToPolkadotAddress(data.request.address)
            );
          });
...
chrome.runtime.onMessage.addListener(async (msg, _sender, sendResponse) => {
  switch (msg.type) {
    ...
    case Messages.ReEncryptPairs:
      ...
      keyPairs = reEncryptKeyringPairs(
        keyPairs,
        msg.payload.oldPassword,
