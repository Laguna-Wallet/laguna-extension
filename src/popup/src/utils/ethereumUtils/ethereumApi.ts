...
export const generateNewWalletAddress = (seed: string): ethers.Wallet => {
  // const pair = keyring.getPair(address);
  // const wallet = ethers.Wallet.fromMnemonic(pair?.meta?.encodedSeed as string);

  const wallet = ethers.Wallet.fromMnemonic(seed as string);
  return wallet;
};
...
export const buildEthereumTransaction = (
  receiverAddress: string,
  amount: string,
  gasPrice: Promise<ethers.BigNumber>,
...
export const getWalletAddress = async (
  ...
  const pair = keyring.getPair(address);
  const decodedSeed = AES.decrypt(pair?.meta?.encodedSeed as string, password);
